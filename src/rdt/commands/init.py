...
def _available_targets() -> list[str]:
    ...
    return (
        [d.name for d in sorted(init_dir.iterdir()) if d.is_dir()]
        if init_dir.exists()
        else []
    )
...
@click.option(
    '--ros-distro', default='jazzy', show_default=True,
    help='ROS 2 distribution.',
)
@click.option(
    '--with', 'include', multiple=True, metavar='TARGET',
    help='Include only these targets (repeatable).',
)
@click.option(
    '--without', 'exclude', multiple=True, metavar='TARGET',
